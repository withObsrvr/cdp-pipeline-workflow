...
// BronzeLedgerReaderProcessor extracts all 19 Bronze table data types from LedgerCloseMeta
// This processor is designed to work with the BronzeCopierConsumer which writes to DuckLake
type BronzeLedgerReaderProcessor struct {
	networkPassphrase string
	subscribers       []Processor
}

// NewBronzeLedgerReaderProcessor creates a new Bronze ledger reader processor
func NewBronzeLedgerReaderProcessor(config map[string]interface{}) *BronzeLedgerReaderProcessor {
	networkPassphrase := "Test SDF Network ; September 2015"
	if np, ok := config["network_passphrase"].(string); ok {
		networkPassphrase = np
	}

	return &BronzeLedgerReaderProcessor{
...
func (p *BronzeLedgerReaderProcessor) Subscribe(processor Processor) {
...
func (p *BronzeLedgerReaderProcessor) Process(ctx context.Context, msg Message) error {
	// Expect LedgerCloseMeta payload
	lcm, ok := msg.Payload.(xdr.LedgerCloseMeta)
	if !ok {
		return fmt.Errorf("BronzeLedgerReader: expected xdr.LedgerCloseMeta, got %T", msg.Payload)
	}
...
func (p *BronzeLedgerReaderProcessor) emit(ctx context.Context, tableType string, record interface{}) error {
	msg := Message{
		Payload: record,
...
func (p *BronzeLedgerReaderProcessor) getLedgerSequence(lcm *xdr.LedgerCloseMeta) uint32 {
	switch lcm.V {
	case 0:
...
func (p *BronzeLedgerReaderProcessor) getClosedAt(lcm *xdr.LedgerCloseMeta) time.Time {
	switch lcm.V {
	case 0:
...
func (p *BronzeLedgerReaderProcessor) getLedgerHeader(lcm *xdr.LedgerCloseMeta) xdr.LedgerHeaderHistoryEntry {
	switch lcm.V {
	case 0:
...
func (p *BronzeLedgerReaderProcessor) getTransactionReader(lcm *xdr.LedgerCloseMeta) (*ingest.LedgerTransactionReader, error) {
	return ingest.NewLedgerTransactionReaderFromLedgerCloseMeta(p.networkPassphrase, *lcm)
}

// Ensure BronzeLedgerReaderProcessor implements Processor interface
var _ Processor = (*BronzeLedgerReaderProcessor)(nil)
